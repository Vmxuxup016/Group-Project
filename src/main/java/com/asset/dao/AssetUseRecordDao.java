...
public class AssetUseRecordDao {
    ...
    public List<AssetUseRecord> findUnreturnedByAssetId(Integer assetId) {
        String sql = "SELECT r.*, a.asset_code AS assetCode, a.asset_name AS assetName, " +
                     "d1.dept_name AS fromDeptName, d2.dept_name AS toDeptName, u.real_name AS operatorName " +
                     "FROM asset_use_record r " +
                     "LEFT JOIN asset a ON r.asset_id = a.id " +
                     ...
                     "LEFT JOIN department d2 ON r.to_dept_id = d2.id " +
                     "LEFT JOIN user u ON r.operator_id = u.id " +
                     "WHERE r.asset_id = ? AND r.return_status = 0 ORDER BY r.id DESC";
        return jdbc.query(sql, new BeanPropertyRowMapper<>(AssetUseRecord.class), assetId);
    }

    public List<AssetUseRecord> findByReturnStatus(Integer returnStatus) {
        String sql = "SELECT r.*, a.asset_code AS assetCode, a.asset_name AS assetName, " +
                     "d1.dept_name AS fromDeptName, d2.dept_name AS toDeptName, u.real_name AS operatorName " +
                     "FROM asset_use_record r " +
                     "LEFT JOIN asset a ON r.asset_id = a.id " +
                     ...
                     "LEFT JOIN department d2 ON r.to_dept_id = d2.id " +
                     "LEFT JOIN user u ON r.operator_id = u.id " +
                     "WHERE r.return_status = ? ORDER BY r.id DESC";
        return jdbc.query(sql, new BeanPropertyRowMapper<>(AssetUseRecord.class), returnStatus);
    }
